"""
Baseline (no-skill) complex drift adoption integration tests.

Mirrors test_complex_drift.py but uses agent_no_skill (no SKILL.md
...
async def test_complex_drift_baseline(
    ...
    """
    Baseline complex drift adoption test (no skill loaded).

    Measures drift reduction rather than requiring full resolution.
    The baseline captures how well raw Claude handles complex drift
    ...
    with drift_test_context(example_name, skip_esc=True) as ctx:
        ...
        # Full resolution is not expected without the skill for complex resources.
